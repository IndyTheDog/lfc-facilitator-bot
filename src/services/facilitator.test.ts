...
const TEAM_CONFIGURATION =
    '{"config":{"team": ["John", "Joe", "Jane"],"second": ["Paul", "Poe", "Pat"]}}';
...
describe('Use the configuration to choose a facilitator', () => {
    ...
    it('selects an existing configuration', async () => {
        ...
        expect(mockPostSlackMessage).toHaveBeenCalledWith("general", ":mega: Cedric requested a facilitator between [John,Joe,Jane].\n*John* is the chosen one :tada:");
    });
    ...
    it('selects a second configuration', async () => {
        ...
        expect(mockPostSlackMessage).toHaveBeenCalledWith("general", ":mega: Cedric requested a facilitator between [Paul,Poe,Pat].\n*Paul* is the chosen one :tada:");
    });
});
